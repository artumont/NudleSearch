...
async def test_post_disabled_success(mock_async_client_cls, mock_response, mock_async_client):
    ...
    mock_async_client.post = AsyncMock(return_value=mock_response(status_code=200, json_data={"status": "ok"}))

    manager = ConnectionManager(proxy_type=ProxyTypes.DISABLED, proxy_pool=None)
    url = "http://test.com"
    data = {"key": "value"}
...
async def test_post_disabled_failure(mock_async_client_cls, mock_response, mock_async_client):
    ...
    mock_async_client_cls.return_value = mock_async_client
    mock_async_client.post = AsyncMock(return_value=mock_response(status_code=404))
    ...
    assert "Proxyless connection failed with status code: 404" in str(excinfo.value)
...
async def test_post_static_success(mock_async_client_cls, mock_check_proxy, mock_response, mock_async_client):
    ...
    mock_async_client_cls.return_value = mock_async_client
    mock_async_client.post = AsyncMock(return_value=mock_response(status_code=200, json_data={"status": "ok_static"}))
...
async def test_post_static_failure(mock_async_client_cls, mock_check_proxy, mock_response, mock_async_client):
    ...
    mock_check_proxy.return_value = True  
    mock_async_client_cls.return_value = mock_async_client
    mock_async_client.post = AsyncMock(return_value=mock_response(status_code=500))
    ...
    with pytest.raises(BridgeException) as excinfo:  
        await manager.post(url, data)
    assert "Static/Rotating connection failed with status code: 500" in str(excinfo.value)
    mock_check_proxy.assert_called_once_with(proxy)
...
async def test_post_rotating_success(mock_async_client_cls, mock_check_proxy, mock_response, mock_async_client):
    ...
    mock_check_proxy.return_value = True  
    mock_async_client_cls.return_value = mock_async_client
    mock_async_client.post = AsyncMock(return_value=mock_response(status_code=200, json_data={"status": "ok_rotating"}))

    proxies = ["http://proxy1.com:8080", "http://proxy2.com:8080"]
    manager = ConnectionManager(proxy_type=ProxyTypes.ROTATING, proxy_pool=proxies)
...
async def test_post_rotating_skips_bad_proxy(mock_async_client_cls, mock_check_proxy, mock_response, mock_async_client):
    ...
    mock_async_client_cls.return_value = mock_async_client
    mock_async_client.post = AsyncMock(return_value=mock_response(status_code=200))
...
async def test_post_bridge_success(mock_async_client_cls, mock_response, mock_async_client):
    ...
    mock_async_client_cls.return_value = mock_async_client
    mock_async_client.post = AsyncMock(return_value=mock_response(status_code=200, json_data={"bridge_status": "ok"}))

    bridge_url = "http://mybridge.com/fetch"
    manager = ConnectionManager(proxy_type=ProxyTypes.BRIDGE, proxy_pool=bridge_url)
...
async def test_post_bridge_failure(mock_async_client_cls, mock_response, mock_async_client):
    ...
    mock_async_client_cls.return_value = mock_async_client
    mock_async_client.post = AsyncMock(return_value=mock_response(status_code=503))
    ...
    assert "Bridge connection failed with status code: 503" in str(excinfo.value)
